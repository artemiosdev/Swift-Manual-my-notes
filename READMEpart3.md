...
Глава . Разработка приложения с использованием UIKit 
#UIKit — это основа любого iOS-приложения, отвечает за создание графического интерфейса, но и за функционирование приложений в принципе. 

Создание проекта MyName 
В Xcode создайте новый проект. В качестве платформы выберите iOS, а в качестве шаблона — App, после чего нажмите Next. 
На экране настроек нового проекта укажите следующие значения:
...
Выбранное в пункте User Interface значение определяет, каким образом будет создаваться интерфейс приложения: средствами UIKit (для этого должно быть выбрано значение Storyboard) или #SwiftUI. При этом выбор одного варианта не отменяет возможности использовать другое средство в процессе работы над проектом. 
Сейчас в Project Editor отображаются настройки проекта. Напомню, что для доступа к этим настройкам в Project Navigator необходимо выбрать корневой объект дерева ресурсов (в данном проекте это элемент MyName с синей иконкой). 
...
Раздел Identity содержит:
-Display – Name название приложения, будет отображаться под иконкой установленного приложения.
-Bundle Identifier – идентификатор продукта. 
-Version – версия приложения.
-Build – номер сборки приложения. 
Раздел Deployment Info позволяет вам изменить перечень поддерживаемых устройств, версию операционной системы, доступную ориентацию (портретный и ландшафтный режимы) и др. 
Прежде чем вы разместите новое приложение в магазине AppStore, его необходимо подписать с помощью цифровой подписи. Возможность (или невозможность) совершения этой операции указана во вкладке Signing & Capabilities. 

 

Структура проекта
На панели Project Navigator приведен текущий состав проекта 
...
-отображения (view «вьюшками») графических элементов сцены (кнопок, надписей, таблиц и т. д.); 
-дополнительные элементы, обеспечивающие функциональность сцены. К таким элементам относятся ограничения (constraints), задающие правила расположения элементов на сцене. 
...
Jump Bar – панель перехода, расположенную прямо над Interface Builder. 
 
Ранее на этапе создания проекта в качестве шаблона был выбран App. По умолчанию проект такого типа в файле Main.storyboard содержит одну сцену, то есть один рабочий экран. 
В левой части Interface Builder находится панель Document outline, описывающая структуру текущего storyboard и каждой отдельной сцены в его составе.
 
Сейчас файл Main.storyboard описывает всего одну сцену, которой в Document outline соответствует элемент View Controller Scene. 
Если Project Navigator (панель в левой части Xcode) позволяет получить доступ к различным ресурсам проекта, в том числе к storyboard-файлам, то благодаря Document outline вы получаете возможность доступа к отдельным элементам storyboard и каждой сцены. 
Элемент View Controller Scene, описывающий сцену, состоит из: 
...
Вы уже встречались с тем, что одни View входят в состав других, когда разрабатывали проект с шариками. Там родительским отображением (view) была прямоугольная подложка, а шарики представляли собой дочерние отображения. 
...
First Responder (ответчик) всегда указывает на объект, с которым пользователь взаимодействует в данный момент. С iOS-приложениями пользователь потенциально может начать взаимодействие со многими элементами, расположенными на сцене. Например, если он начал вводить данные в текстовое поле, то именно оно и будет являться First responder. 
...
Storyboard Entry Point – это признак стартовой (или инициирующей) сцены приложения. Сцена, отмеченная такой стрелкой, будет отображена первой после завершения загрузки приложения. 
...
-(2) Перетащите его прямо из библиотеки в центр сцены в Interface Builder. При этом во время позиционирования на сцене будут отображаться синие вертикальная и горизонтальная линии, пересечение есть центр. 
...
В действительности класс UIButton наследуется от UIControl, который, в свою очередь, наследуется от UIView. Все графические элементы представляют собой довольно большую иерархию классов, каждый из которых дополняет родителя некоторыми возможностями. 

  
Шаг 2. Изменение текста кнопки 
Для изменения текста есть два варианта действий. Вариант 1: 
-Дважды щелкните по кнопке на сцене и введите текст "Hello World". 
...
После изменения текста вам может потребоваться повторная центровка кнопки на сцене, так как ее размер изменился. 
  
Запуск приложения в симуляторе
Доступ к симуляторам предоставляет пункт меню Xcode -> Preferences -> Components. Уже скачанные симуляторы отмечены синей галочкой, а доступные для загрузки — серой стрелочкой.
В качестве устройства выберите один из доступных iPhone 
 
Если запустить на др версии устройства к примеру на Iphone 7 и тп, то надпись будет не по центру. Так как изначально мы ее позиционировали под др устройство, и указали конкретные координаты. Исправим это
...
-В открывшемся окне выделите пункты Horizontally in Container и Vertically in Container. 
-Нажмите кнопку Add 2 Constraints. 
Вы только что добавили ограничение (constraint), которое заставит кнопку автоматически позиционироваться в центре экрана любого запущенного устройства. С помощью ограничений можно определять различные правила позиционирования элементов: отступы друг от друга, отступы от границ экрана, выравнивание и др
...
Класс ViewController связан с элементом View Controller на сцене. Именно с его помощью мы и будем управлять сценой. Чтобы увидеть эту связь, выполните следующие действия: 
-Выделите файл Main.storyboard в Project Navigator. 
-Выделите элемент View Controller в составе сцены в Document Outline. 
-Откройте панель Identity Inspector. 
-В поле Class данного элемента будет указан класс ViewController, который как раз и описан в файле ViewController.swift. 
...
Вернемся к файлу ViewController.swift. Импорт фреймворка UIKit
import UIKit 
UIKit обеспечивает функциональные возможности для построения и управления пользовательским интерфейсом, анимацией, текстом, изображениями для ваших приложений, а также для обработки событий, происходящих во время работы пользователя. UIKit – это набор основных возможностей, которые разработчики используют при построении интерфейса практически любого приложения. 
...
Сейчас же в данном методе нет необходимости, поэтому уберем его из кода, оставив пустой класс ViewController 
class ViewController: UIViewController {}

Доступ UI к коду. Определитель типа @IBAction 
Добавим в наше приложение функциональные возможности, вывод на консоль текстового сообщения при нажатии на кнопку. Ранее мы говорили, что класс ViewController ассоциирован с элементом View Controller сцены. Соответственно, для расширения возможностей сцены необходимо работать именно с этим классом. Объявим в нем новый метод showMessage(), использующий внутри своей реализации функцию print(_:), выводящую строку "you pressed Hello World button". 
Вы можете использовать кодовые сниппеты для упрощения создания кодовых конструкций. 
class ViewController: UIViewController {
     func showMessage(){
...
Обратите внимание, что в редакторе кода слева от метода showMessage() вместо номера строки теперь отображается круг. 
...
IBAction расшифровывается как Interface Builder Action, то есть действие, к которому можно обратиться из Interface Builder. 
Так как метод showMessage() помечен с помощью экшена, мы можем организовать его вызов по событию «нажатие на кнопку». Для этого: 
-Откройте файл Main.storyboard. 
-Удерживайте клавишу Control на клавиатуре и с помощью мыши потяните кнопку сцены на желтый кружок, символизирующий View Controller, после чего отпустите. В процессе должна отображаться линия синего цвета. 
-В появившемся окне выберите элемент showMessage, находящийся в разделе Sent Events.
Метод showMessage() находится в выпадающем списке и доступен для связывания, так как помечен ключевым словом @IBAction. 
Теперь если запустить приложение и щелкнуть на кнопке, то будет вызван метод showMessage() класса ViewController, а значит, произойдет вывод фразы "you press Hello World button" на консоль.
 

 

Создание дополнительной сцены 
Xcode предоставляет вам множество способов создания сцен и организации перехода от одной сцены к другой. Вы можете делать это программным кодом или использовать возможности Interface Builder. Мы займемся созданием 2 сцены и переход к ней по нажатии кнопки. 
-На сцене под кнопкой Hello World разместите новую кнопку и поменяйте ее текст на «Show Next Screen». 
-В библиотеке объектов найдите элемент View Controller и перетащите его на storyboard, разместив справа от существующей сцены. 
Теперь storyboard проекта содержит две сцены, и мы можем организовать переход между ними. Но сперва внесем во вторую сцену отличительную особенность, изменим цвет ее фона: 
1.В Document outline выделите элемент View новой сцены. 
2.Откройте панель Attributes Inspector. 
3.Поменяйте значение свойства background
Нам осталось только добавить переход от первой сцены ко второй: 
-Зажмите клавишу Control на клавиатуре и с помощью мыши перетяните кнопку Show Next Screen на новую сцену. При этом сама кнопка останется на месте, а от нее будет тянуться синяя линия. 
-Когда новая сцена подсветится синим, отпустите кнопку мыши, а в появившемся меню выберите Show в разделе Action Seque. 
Переход между сценами создан! Обратите внимание, что на storyboard появилась серая стрелка, идущая от одной сцены к другой, а в Document Outline теперь отображается новый элемент Show segue to View Controller 
...
Класс ViewController, объявленный в файле ViewController.swift, связан с первой сценой приложения. С его помощью обрабатывается нажатие кнопки Hello World и выводится текст на консоль. В дальнейшем и новая сцена получит некоторые функциональные возможности. Для этого требуется связать ее с собствен классом, в котором будут обрабат события: 
-В Project Navigator щелкните правой кнопкой мыши и выберите New File. 
-В появившемся окне выберите Cocoa Touch Class и нажмите Next 
   
В новом окне в качестве имени класса укажите SecondViewController, а в качестве родительского класса (поле Subclass of) — UIViewController. Далее нажмите Next и подтвердите сохранение файла. В Project Navigator появился новый файл Project Navigator.swift. Если открыть его, то в нем вы найдете новый класс SecondViewController. Свяжем его с новой сценой: 
...
Будьте внимательны: выбирайте View Controller именно новой (красной) сцены. Теперь новая сцена связана с новым классом и при необходимости мы можем обеспечить обработку события, например нажатия кнопок. 
 
Отображение всплывающего окна. Класс UIAlertController 
...
В состав UIKit входит класс #UIAlertController, который позволяет организовать вывод всплывающих окон на экран устройства. Такой вид оповещения пользователя может быть отображен в классическом виде в центре экрана (тип Alert) или в нижней части сцены (тип Action Sheet) 
