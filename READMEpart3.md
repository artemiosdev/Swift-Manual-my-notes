...
**Структуры — это value type (значимые типы) «дубликат».*8
...
Тип Date ранее не рассматривался и не использовался в книге. Он предназначен для работы с датой и временем. В данном случае с его помощью получается число, описывающее количество секунд, прошедших с 1 января 1970 года, так называемое Unix-время. Для его работы требуется импортировать библиотеку Foundation. 
...
-Тип UserController UserController – это контроллер, менеджер пользователей, производящий управление учетными записями, для которого бессмысленно использовать преимущества структуры. Вполне логично, если значение данного типа всегда будет передаваться по ссылке. 
struct User { 
var id: Int } 
...
Универсальные шаблоны в Swift представлены 3 базовыми элементами: 
...
метод #pop( ) — возвращает первый элемента стека, удаляя его оттуда. Наличие других свойств и операций не подразумевается. 
...
Ограничения доступны и при работе с универсальными типами. Показан пример стека, который может хранить только числовые значения. 
...
T1 должен быть Comparable, а T2 должен быть Collection. 
...
Как  будет выглядеть протокол StackProtocol, который станет основой для универсального типа Stack. 
...
2 и 3 варианты описывают ошибки (исключительные ситуации, или исключения), из-за которых невозможно выполнить требуемую операцию.
...
Реализуем функцию, которая в автоматическом режиме пытается приобрести какой-либо товар. В данном примере словарь favoriteSnacks со- держит указатель на любимое блюдо каждого из трех человек. 
let favoriteSnacks = [
    "Alice": "Chips",
    "Bob": "Licorice",
    "Eve": "Pretzels",
