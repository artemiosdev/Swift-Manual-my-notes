...
Сhanging constraints напрямую не приводит к обновлению макета. Чтобы оживить вызов `layoutIfNeeded`  необходим, чтобы заставить механизм layout обновить размер и положение views внутри анимационного блока.
...
<img alt="image" src="images/auto layout105.jpeg" width = 60%/>

<img alt="image" src="images/auto layout106.jpeg" width = 60%/>

We need an outlet in the view controller that connects to the vertical constraint in Interface Builder. Using the Assistant Editor controldrag from the centerY constraint in Interface Builder to the view controller to create the connection: 
`@IBOutlet var centerConstraint: NSLayoutConstraint!`

Когда view загружается впервые, я хочу расположить кнопку, используя смещение offset. Я делаю это, устанавливая постоянное значение вертикального ограничения в viewDidLoad.
...
Когда view появляется на экране, я анимирую его, возвращая в центральное положение без смещения по вертикали.
...
В методе `animateButton` мы сначала изменяем постоянное значение ограничения по центру. Установка значения в 0 перемещает кнопку обратно в вертикальный центр экрана.

Внутри блока анимации мы вызываем `layoutIfNeeded`, чтобы заставить layout механизм обновить положение кнопки.
