...
Экземпляры class являются изменяемыми mutable объектами, в то время как экземпляры struct являются неизменяемыми immutable значениями. Следующий пример иллюстрирует это различие: Grade – школьная оценка
struct Grade {
  let letter: String
  let points: Double
  let credits: Double
...
Значение “reference1” красным цветом – это значение, сохраненное в jane. Это значение является ссылкой, и поскольку jane объявлена как константа, эта ссылка является постоянной и constant. Если бы вы попытались назначить jane другого ученика, вы получили бы ошибку компилятора:
