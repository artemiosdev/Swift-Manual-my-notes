...
- протокол-ориентированное программирование (ПОП). Более подробнее в [Глава  №39](https://github.com/artemiosdev/Swift-Manual-my-notes/blob/main/READMEpart3.md#chapter39) 
...
- `as?` (#as?) ИмяКласса возвращает либо экземпляр типа ИмяКласса? (опционал), либо nil в случае неудачного преобразования; 
...
Finally, it’s important to understand that classes and class hierarchies model what objects are. If your goal is to share behavior (what objects can do) between types, more often than not you should prefer protocols over subclassing. 
...
Для контроля доступа используются специальные модификаторы, которые указываются перед объявлением объектного типа, свойства и метода. [Всего доступны 5 модификаторов:](https://swiftbook.ru/content/languageguide/access-control/) 
...
- Подобно кортежу, уровень доступа к функции определяется самым строгим уровнем типов аргументов функции и типа возвращаемого значения. 
...
#Инициализатор (конструктор) – это специальный метод, выполняющий подготовительные действия при создании экземпляра объектного типа данных. 

**Инициализатор** срабатывает при создании экземпляра, а при его удалении вызывается деинициализатор. 
...
Не забывайте, что в случае использования проваливающегося инициализатора возвращается опционал. 
Поэтому прежде, чем работать с экземпляром, необходимо выполнить извлечение опционального значения. 
...
###  <a id="chapter28" /> Глава №28. Управление памятью в Swift. Stack and heap
...
**Типы значений** - копии экземпляров типов значений являются самостоятельными объектами, каждый из которых занимает отдельное место в памяти устройства Примеры: Базовые типы данных (String, Int, Bool), структуры, перечисления, массивы, словари и т.д.
...
Объекты, записанные в стек, похожи на стопку тарелок: последняя поставленная тарелка не дает убрать те, что находятся ниже, пока данная тарелка сама не будет убрана. 
...
В стеке хранятся значимые типы данных (value type), а также указатели на ссылочные типы (reference type). Работая со стеком, вы не встретитесь с какими-либо проблемами излишнего использованием памяти: его элементы своевременно создаются и удаляются. 
...
if true { // шаг 1
    let houseOwner = Human(name:"Artem")
    if true { // шаг 2
...
1. В куче выделяется память под новый объект ссылочного типа, описывающий владельца (Artem). В стеке создается новый фрейм, в котором выделяется память под константу houseOwner, содержащую указатель на объект «Artem» в куче. 
...
6. Оператор if завершает свою работу, а значит, происходит выход из соответствующей области видимости. По этой причине последовательно удаляются элементы фрейма #1 в стеке. После этого удаляется объект House (Квартира), так как на него больше нет ссылок. В последнюю очередь удаляется объект типа Human (Artem), так как последняя ссылка на него исчезла вместе с House (Квартира). 
...
Если посмотреть на вывод на консоли, то вы увидите, что все три объекта (`houseOwner, house, secondHouse`) были созданы. Но хотя код и завершил работу без ошибок, они так и не были удалены (деинициализатор ни в одном из них не был вызван). Это пример утечки памяти: происходил выход из областей видимости, но локальные параметры не удалялись. 

Для того чтобы разобраться, рассмотрим по шагам: 
1. В куче выделяется память под новый объект ссылочного типа, описывающий владельца (Artem). В стеке создается новый фрейм, в котором выделяется память под константу houseOwner, содержащую указатель на данный объект. 
...
Подобная ситуация называется циклом сильных strong ссылок retain cycle. В этом случае Swift просто не может решить, какую из ссылок можно уничтожить, а какую нет. В результате все объекты имеют входящие ссылки и не удаляются из памяти. 
...
1. В куче выделяется память под новый объект ссылочного типа, описывающий владельца (Artem). В стеке создается новый фрейм, в котором выделяется память под константу houseOwner, содержащую ссылку на объект «Artem». 
...
Because in Swift rely on to remove them from memory, it’s important to understand the concept of a.

Add a field representing a— for example, a lab partner — and a deinitializer to like this:

Поскольку classes в Swift полагаются на reference counting, чтобы удалить их из памяти, важно понять концепцию цикла сохранения retain cycle.

Добавьте поле, представляющее одноклассник classmate, например,  партнер — и deinitializer для class Student:
...
Теперь предположим как Алиса так и Боб выбрались из школы:
...
Если вы запускаете это на вашей playground, вы заметите, что вы не видите сообщения «Alice/Bob is being deallocated!» и Swift не вызывает deinit. Почему это?

Alice and Bob каждый имеют ссылку друг на друга, поэтому отсчет никогда не достигает нуля! Чтобы сделать вещи хуже, назначив nil Алису и Бобу, больше ссылок на исходные объекты. Это классический случай цикла сохранения retain cycle, который приводит к программному багу, известному как утечка памяти memory #leak.

С утечкой памяти, сама память не освобождается, даже если его практический жизненный цикл закончился. Retain cycles являются наиболее распространенной причиной утечек памяти. К счастью, есть способ, чтобы объект Student мог ссылаться на другого Student, не склоняясь retain cycles сохранять циклы, это делает ссылку reference #weak:
...
Эта простая модификация отмечает переменную partner как **weak, что означает ссылку в этой переменной не примет участие в reference counting. Когда ссылка не weak, это называется strong ссылкой, которая является по умолчанию в Swift. Слабые ссылки должны быть объявлены как optional types, так что, когда объект, на который они ссылаются освобождается, автоматически становится nil**.

### Ссылки в замыканиях 

В процессе изучения материала мы неоднократно говорили о том, что value type передается копированием, а reference type – по ссылке. И это утверждение истинно практически в 100% случаев, но если на одном из будущих собеседований вас спросят: «А может ли value type передаваться по ссылке?» и вы ответите: «Нет! Не может!», то, вероятно, не пройдете его. В этом разделе я покажу вам простейший пример того, что и значимые типы могут быть переданы по ссылке. 
...
Такое поведение также может быть для вас неочевидным и стать источником проблем, в частности, в результате несвоевременного удаления объектов, захваченных в замыканиях. Но и в этом случае можно использовать модификаторы `weak` и `unowned` для изменения типа ссылки для входных аргументов замыканий. 

Рассмотрим пример, в котором создается пустое опциональное замыкание и в зоне ограниченной области видимости (в теле оператора if) ему передается значение (тело замыкания). 
...
Для решения проблемы в замыкании необходимо выполнить захват переменной, указав при этом, что в переменной содержится слабая ссылка 
...
При необходимости указать тип ссылки для нескольких входных аргументов это нужно делать перед каждым из них отдельно.
